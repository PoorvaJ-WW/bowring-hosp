...
// JSON-DRIVEN TeamWithLargeImages Template - Simple Direct Editing
'use client';
...
const getComponentContent = (pageSlug: string, componentId: string) => {
  ...
  return page.components?.find((c: any) => 
    c.id === componentId || c.type === 'team-with-large-images'
  );
};

export default function TeamWithLargeImagesAcademicsAndResearchAcademicsAndResearch(props: Props) {
  const {
    pageSlug = 'home',
    componentId = 'team-with-large-images',
    theme: propTheme,
    backgroundType,
    previewContent
  } = props;
  ...
  if (previewContent) {
    ...
    if (!pageSlug) {
      console.error('TeamWithLargeImages: pageSlug is required when previewContent is not provided');
      return null;
    }
    ...
    if (!content || Object.keys(content).length === 0) {
      console.error(`TeamWithLargeImages: content not found for ${pageSlug}/${componentId}`);
      return null;
    }
  }
  ...
  const {
    ...
    description = 'Our dedicated team members who make it all possible',
    members = [
      {
        name: 'Team Member 1',
        role: 'Position',
  ...
  const bodyTextColor = getDescriptionColor({ theme: currentTheme }); // For subtitle and member roles
            ...
            return (
              <li key={member.name || `member-${index}`}>
                <div className="rounded-md shadow">
                  ...
                  {false && (
                    <div
                      className={`aspect-3/2 w-full rounded-2xl bg-gray-100 flex items-center justify-center ${getEditableClasses()}`}
                      {...getEditableProps(
                        (e) => {
                          e.stopPropagation();
                ...
                <h3
                  className={`mt-6 text-lg/8 font-semibold tracking-tight transition-colors duration-200 ${getEditableClasses()}`}
                  style={{
                    color: headingColor,
                    fontFamily: headingFont
                  }}
