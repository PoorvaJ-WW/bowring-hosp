...
// JSON-DRIVEN FaqTwoColumnsCenter Template - Simple Direct Editing
'use client';
...
const getComponentContent = (pageSlug: string, componentId: string) => {
  ...
  return page.components?.find((c: any) => 
    c.id === componentId || c.type === 'faq-two-columns-center'
  );
};

export default function FaqTwoColumnsCenterPatientCarePatientCare(props: Props) {
  const {
    pageSlug = 'home',
    componentId = 'faq-two-columns-center',
    theme: propTheme,
    backgroundType,
    previewContent
  } = props;
  ...
  if (!previewContent && (!content || Object.keys(content).length === 0)) {
    console.error(`FaqTwoColumnsCenter: content not found for ${pageSlug}/${componentId}`);
    return null;
  }
