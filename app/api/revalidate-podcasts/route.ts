...
import { WEBSITE_ID } from '@/lib/podcasts';

// Secret token for authorization - uses global prefix + content type + website ID
// Format: john1010-jesus-ws-podcasts-{WEBSITE_ID}
const REVALIDATE_SECRET = `john1010-jesus-ws-podcasts-${WEBSITE_ID}`;

/**
 * Next.js 15 On-Demand Revalidation API for Podcasts
 * This endpoint allows the main dashboard to trigger cache revalidation
 * when podcast content is updated, deleted, or published
 *
 * Usage from dashboard:
 * POST /api/revalidate-podcasts
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pathsToRevalidate = paths || ['/podcasts', '/', '/podcasts/[slug]'];
    ...
    const tagsToRevalidate = tags || ['podcasts', `website-${websiteId}`];
