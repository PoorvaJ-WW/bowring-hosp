...
import { WEBSITE_ID } from '@/lib/posts';

// Secret token for authorization - uses global prefix + content type + website ID
// Format: john1010-jesus-ws-posts-{WEBSITE_ID}
const REVALIDATE_SECRET = `john1010-jesus-ws-posts-${WEBSITE_ID}`;

/**
 * Next.js 15 On-Demand Revalidation API for Posts
 * This endpoint allows the main dashboard to trigger cache revalidation
 * when post content is updated, deleted, or published
 *
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pathsToRevalidate = paths || ['/posts', '/', '/posts/[slug]'];
    ...
    const tagsToRevalidate = tags || ['posts', `website-${websiteId}`];
